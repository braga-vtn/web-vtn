...
import { fetchBlingData } from '../../../_actions/blingRequest';

interface CardBlingProps extends NodeProps {
    ...
    <>
      ...
      >
        <CardHeader className="flex flex-row items-center gap-4">
          <Avatar>
            <AvatarImage src="/avatar-bling.jpg" alt="avatar-bling" />
        ...
        <CardContent>
          {demo || (blingData && blingData.connectedBling) ?
            <FloatingLabelInput value={blingData?.connectedBling && blingData ? blingData.shopBling : "Vistune"} disabled={true} id="shop-bling" label="Nome da Loja" />
            :
            <Alert>
        ...
        <div className={`absolute left-3 top-4 h-2 w-2 rounded-full ${demo || blingData ? demo || (blingData?.connectedBling && isConnected) ? "bg-green-500" : !blingData?.connectedBling && isConnected ? "bg-yellow-400" : "bg-red-600" : isConnected ? "bg-green-500" : "bg-red-600"}`}>
        </div>
      </Card>
    </>
  );
...
const CardBlingWrapper: React.FC<NodeProps & DemoProps & { onDeleteNode: (id: string) => void }> = (props) => {
  return <CardBling {...props} onDeleteNode={props.onDeleteNode} demo={props.demo} />;
};

export default CardBling;
export { CardBlingWrapper };