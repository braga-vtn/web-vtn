...
import { fetchControlInstagramData, updateControlInstagramData } from '../../../_actions/controlInstagramRequest';
...
const ControlInstagram = ({ node, onExample }: Props) => {
  ...
  const [instagramData, setInstagramData] = useState<{ accountInstagram: string; connectedInstagram: boolean } | null>(null);
  ...
  useEffect(() => {
    fetchControlInstagramData().then(data => {
      setSelectValues({
  ...
  const handleSave = () => {
    ...
    updateControlInstagramData(data);
  ...
  return (
    <div>
      {instagramData?.connectedInstagram ?
        <>
          ...
          <CardContent>
            <Alert>
              ...
              <AlertDescription className='text-muted-foreground text-xs'>
                Seu Instagram ainda não está integrado à Vistune.
              </AlertDescription>
            </Alert>
          </CardContent>
          <CardFooter className="mt-2">
...
export default ControlInstagram;