...
interface CardYoutubeInsightsProps extends NodeProps {
  data: EditorCanvasCardType;
  demo: boolean;
  onDeleteNode: (id: string) => void;
}

const CardYoutubeInsights: React.FC<CardYoutubeInsightsProps> = ({ data, demo, onDeleteNode }) => {
    ...
    <>
      ...
      >
        <CardHeader className="flex flex-row items-center gap-4">
          <Avatar>
            <AvatarImage src="/avatar-youtube-insights.jpg" alt="avatar-youtube-insights" />
        ...
        <CardContent>
          {demo || (youtubeInsightsData && youtubeInsightsData.connectedYoutubeInsights) ?
            <FloatingLabelInput value={youtubeInsightsData?.connectedYoutubeInsights && youtubeInsightsData ? youtubeInsightsData.accountYoutubeInsights : "Vistune YT"} disabled={true} id="account-youtube-insights" label="Nome da Conta" />
            :
            <Alert>
              <AlertCircle className="h-4 w-4" />
              <AlertTitle>Não está Conectado!</AlertTitle>
              <AlertDescription className='text-muted-foreground text-xs'>
                Sua conta do Youtube Insights não está integrada à Vistune.
              </AlertDescription>
            </Alert>
          }
        </CardContent>
        <div className={`absolute left-3 top-4 h-2 w-2 rounded-full ${demo || youtubeInsightsData ? demo || (youtubeInsightsData?.connectedYoutubeInsights && isConnected) ? "bg-green-500" : !youtubeInsightsData?.connectedYoutubeInsights && isConnected ? "bg-yellow-400" : "bg-red-600" : isConnected ? "bg-green-500" : "bg-red-600"}`}>
        </div>
      </Card>
    </>
  );
...
const CardYoutubeInsightsWrapper: React.FC<NodeProps & DemoProps & { onDeleteNode: (id: string) => void }> = (props) => {
  return <CardYoutubeInsights {...props} onDeleteNode={props.onDeleteNode} demo={props.demo} />;
};

export default CardYoutubeInsights;
export { CardYoutubeInsightsWrapper };